...
export const getBibliographic = (html: any) => {
  ...
  const bibliographic = {
    ...
    publishDate: pubTableData.split('(65) 공개번호/일자 ')[1] !== undefined && pubTableData.split('(65) 공개번호/일자 ')[1].split(' (')[1] !== undefined ? pubTableData.split('(65) 공개번호/일자 ')[1].split(' (')[1].replace(/\./g, '-').replace(')', '').replace(/\n/g, '').replace(/\t/g, '').replace(/  /g, '') : '', // 공개일자
    // 공고번호
    // 공고일자
