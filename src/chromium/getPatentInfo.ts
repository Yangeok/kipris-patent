...
async function getListData(page: Page, params: { 
  ...
  await summaries.reduce(async (prevPromise, i) => {
    ...
    const result = {
      ...
      cpcs: details?.bibliographic.cpcs,
      ipcs: details?.bibliographic.ipcs,
      ...
      citating: details?.citating !== '[null]' ? details?.citating : '',
      citated: details?.citated !== '[null]' ? details?.citated : '',
      familyPatents: details?.familyPatents !== '[null]' ? details?.familyPatents : ''
...
async function getDataDetails(params: { 
  ...
  try {
    ...
    const bibliographic = {
      // TMP: 
      ipcs: tableData[0].split(')').map(i => i.split('(')[0]).join(', '),
      cpcs: tableData[1].split(')').map(i => i.split('(')[0].substr(1)).join(', '),
    ...
    const citating = citatingValues
      .map(i => i.length > 1 ? fromEntries(i.map((j, index) => ([ params.citatingFields[index], j ]))) : null)
    ...
    const citated = citatedValues
      .map(i => i.length > 1 ?fromEntries(i.map((j, index) => ([ params.citatedFields[index], j ]))): null)
    ...
    const familyPatents = familyPatentValues
      .map(i => i.length > 1 ? fromEntries(i.map((j, index) => ([ params.familyPatentFields[index], j]))) : null)
      ?.map(i => i?.familyNumber)?.join(', ') // TMP: 
